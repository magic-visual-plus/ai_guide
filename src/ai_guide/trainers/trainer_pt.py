...
def run_train(data_path, device, num_epochs=100):
    train_dataset = ex.PointTransformerDataset(os.path.join(data_path, 'train'), size=512, voxel_size=0)
    val_dataset = ex.PointTransformerDataset(os.path.join(data_path, 'val'), is_test=True, voxel_size=0)
    ...
    train_loader = torch.utils.data.DataLoader(
        train_dataset, batch_size=2, shuffle=True, collate_fn=ex.collate_fn_pt, num_workers=16)
    val_loader = torch.utils.data.DataLoader(
        val_dataset, batch_size=2, shuffle=False, collate_fn=ex.collate_fn_pt, num_workers=16)
    ...
    for iepoch in range(num_epochs):
        ...
        for i, (x, feat, label, offset) in enumerate(train_loader):
            x = x.to(device)
            feat = feat.to(device)
