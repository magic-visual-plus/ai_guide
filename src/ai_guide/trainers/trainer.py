...
def run_train(data_path, device, num_epochs=100):
    train_dataset = ex.PointNetDataset(os.path.join(data_path, 'train'), size=512, voxel_size=0)
    val_dataset = ex.PointNetDataset(os.path.join(data_path, 'val'), is_test=True, voxel_size=0)
    ...
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=2, shuffle=True, collate_fn=ex.collate_fn, num_workers=16)
    val_loader = torch.utils.data.DataLoader(val_dataset, batch_size=2, shuffle=False, collate_fn=ex.collate_fn, num_workers=16)
    ...
    for iepoch in range(num_epochs):
        ...
        for i, (x, x_sampled, group_index, knn_index, labeled, mask, mask_sampled) in enumerate(train_loader):
            ...
            mask_low_proba = ((labeled == 1) & (proba <= 0.9)) | ((labeled == 0) & (proba > 0.1))
...
def run_train_hierachical(data_path, device, num_epochs=100):
    train_dataset = ex.HierarchicalPointNetDataset(os.path.join(data_path, 'train'), size=256, layers=[0])
    val_dataset = ex.HierarchicalPointNetDataset(os.path.join(data_path, 'val'), size=32, is_test=True, layers=[0])

    train_loader = torch.utils.data.DataLoader(
        train_dataset, batch_size=3, shuffle=True, collate_fn=ex.collate_fn, num_workers=16,
        persistent_workers=True, prefetch_factor=64)
    val_loader = torch.utils.data.DataLoader(
        val_dataset, batch_size=3, shuffle=False, collate_fn=ex.collate_fn, num_workers=16,
